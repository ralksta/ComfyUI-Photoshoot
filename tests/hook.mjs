...
//
// shared.mjs imports the ComfyUI front end from "../../scripts/app.js" (and
// api.js), which only exist inside a running ComfyUI. So the test builds a throwaway tree
...
// shared.mjs also imports the API client (outfit previews queue through it);
// the hook does not touch it, so an empty stand-in is enough.
fs.writeFileSync(path.join(tmp, "scripts", "api.js"), "export const api = {};\n");
