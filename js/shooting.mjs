  ...
  d.felder.forEach((f, platz) => {
    ...
    if (f.quelle === "kamera") {
      ...
      if (ordnung === "reihum") {
        ...
        kamera = (reihe.length ? reihe : erlaubt)[lauf % (reihe.length || erlaubt.length)];
        ...
        const k = n > 1 ? Math.floor((2 * i * (reihe.length - 1) + (n - 1)) / (2 * (n - 1))) : 0;
        kamera = reihe[k];
      } else {
...
function baue(node, d) {
  ...
  function achse({ schluessel, name, stand, hinweis, inhalt, zusatz }, state) {
  ...
  function zeichne() {
    ...
    const detailWort = { voll: uet("nah", "ui"), figur: uet("Figur", "ui"), identitaet: uet("weit", "ui") };
