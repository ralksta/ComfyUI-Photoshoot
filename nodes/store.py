...
class Krea2PromptJoin:
    ...
    DESCRIPTION = ("Ersetzt {person1} {person2} {person3} {kamera} {pose} {ausdruck} "
                   "{szene} {stil} {extra}. Empfohlen: {kamera}, {szene}, {person}, "
                   "{pose}, {extra}, {ausdruck}, {stil}. {extra} optional für "
                   "Erweiterungs-Packs. Ohne Platzhalter: Kamera/Szene vor Person.")

    def join(self, text, person_1="", person_2="", person_3="", kamera="",
             pose="", ausdruck="", szene="", stil="", extra=""):
        ...
        if any("{%s}" % p in basis for p in PLATZHALTER):
            ...
            # No placeholder: framing and room before the person. It used to
            # put the person first and the scene last - and then the person
            # block wins the frame area, even on a wide shot.
            # {extra} sits between pose and expression: whatever a pack adds
            # belongs behind the posture and in front of the face.
            vorn = [werte[p] for p in ("kamera", "szene",
...
#
# Naheliegender waere NODE_CLASS_MAPPINGS["Krea2%sSave" % art.capitalize()], und
# genau so stand es hier. Zur Laufzeit ist das gleichwertig, aber ComfyUI-Manager
# liest die Node-Namen statisch aus dem Syntaxbaum, um extension-node-map.json zu
# bauen - die Tabelle, aus der "Install Missing Custom Nodes" ableitet, welches
# Paket einen fehlenden Node liefert. Ein berechneter Schluessel ist dort nicht
# lesbar: der Scanner fand 6 von 14 Nodes, die acht Ablage-Nodes fehlten. Wer
# einen geteilten Workflow mit "Krea2PersonSave" oeffnet, bekaeme kein Angebot.
#
# Die Namen duerfen sich nicht aendern - sie stehen in jedem gespeicherten
# Workflow. tests/smoke.py prueft, dass jeder Schluessel woertlich im Quelltext
# steht und dass die Menge genau der ist, die die Schleife erzeugt.
