...
def _val(cat, label, kopie=None):
    ...
    for lbl, wert in PRESETS[cat]:
        if lbl == label:
            return wert
    # A custom entry (eigene.py), else the label as before.
    return eigene.wert("lighting", cat, label, kopie) or label


def _ziehe(cat, seed, erlaubt=None):
    # Custom entries only from a file marked "dice": true (see pose_builder).
    labels = erlaubt if erlaubt else [lbl for lbl, _ in PRESETS[cat]] + eigene.wuerfelbar("lighting", cat)
