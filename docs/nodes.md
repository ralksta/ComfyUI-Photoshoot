...
| Node | Purpose |
|---|---|
| **Photoshoot Person** | 49 fields across six tabs: basics, body, head, face, make-up, clothing. Outputs `person` (full text) and `person_data` (JSON, for camera-dependent shortening in Photoshoot) |
...
The 72 shoe models are chosen by family and then model; the largest family has
eleven entries. Before that it was a block of chips covering all of them, which
alone took 130 px and made *Extras* the one tab at 346 px that did not fit the
panel; before that, a dropdown with every model in it, which was unusable in its
own way. Top and bottom work the same way: family first, then the cut.

Top and bottom sit on the Clothing tab above hosiery and shoes. The top has
...
The bottom has a third field, **material** (17: latex, patent, leather,
...
The superscript number on each tab says how many fields are set there — with 49
...
The widget under the title is the **photo number**, not a seed. It was called
`seed` until 2.3.0, and the first thing people did was set it to *randomize*
and ask why the series no longer counted. It is 1-based: photo 1 is the first,
and the preview in the panel shows the same numbers. The noise seed is derived
from it and leaves on the `bildseed` output.

Next to the photo count sits the **start number** with a die. *Start shooting*
sets the counter to it, so a series can begin at 500 as easily as at 1 — and a
new start number is a new series with the same settings, which is the answer
to "how do I get different photos without changing the prompt". Type the
number back in and the series comes back. The die draws six digits, so the
number stays readable in the widget.
...
```text
{style}, {camera}, {scene}, {lighting}, {person}, {pose}, {extra}, {expression}
```
