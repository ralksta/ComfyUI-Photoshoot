...
[![Changelog](https://img.shields.io/badge/changelog-v2.4.0-blue)](CHANGELOG.md)
...
> [!IMPORTANT]
> **Top 5 Changes in Release 2.4.0:**
> 1. **Top field:** the Clothing tab gets a top with its own colour, starting with twelve crop tops — each wording kept only because it rendered differently from a bare "crop top".
> 2. **Bottom field:** 29 skirts, shorts and trousers in two families, with colour, written right after the top (`wearing a pastel blue sleeveless crew-neck crop top in tight vertical rib knit, with a grey mini skirt`).
> 3. **Bottom material:** 17 fabrics from high-gloss latex to corduroy, placed between colour and cut; cuts that name their own fabric (denim, leather, tulle) ignore it.
> 4. **72 shoes:** six more presets — ankle-strap and d'Orsay pumps, heeled oxfords, T-strap platforms, chelsea boots, d'Orsay flats — appended so the shoe poster keeps its order.
> 5. **Three posters:** the shoe matrix now shows all 72, and new [Tops](docs/POSTERS.md#tops-matrix-12-crop-tops) and [Bottoms](docs/POSTERS.md#bottoms-matrix-29-skirts-shorts--trousers) matrices show every cut on the README's person, same seed and scene.
>
...
49 fields across six tabs. You pick labels; the node writes English. Related fields
...
tab; top, bottom, hosiery and shoes carry their colour as a separate field on
the same row, top and bottom after a family choice like the shoes; the reset
buttons name what they would clear. The composed English sentence sits at the
bottom and updates as you click.*
...
1. **Build a person.** 49 fields across six tabs. Save under a name and reuse
...
2. **Send them to a photoshoot.** Set the count, restrict the axes you care
   ...
   not the Queue button above the canvas. Queue renders one run, whatever
   photo number the widget shows; *Start shooting* sets the counter to the
   start number and queues the whole series. A new start number is a new
   series with the same settings — the die next to it draws one.
...
| Node | What it does |
|---|---|
| **Photoshoot Person** | 49 fields on six tabs; outputs the person as text and as JSON |
...
start number next to the photo count, or click the die beside it, and press
*Start shooting*. Photos 500 to 511 are a different series from photos 1 to
12, and just as reproducible. Setting the widget's control to *randomize*
works too, then every photo is a random draw — but combinations can repeat,
and there is no series to come back to.
