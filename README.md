...
[![Reference Posters](https://img.shields.io/badge/reference_posters-8_matrices-orange)](docs/POSTERS.md)

![Photoshoot — one person, a whole series, ComfyUI](docs/banner.png)

> [!IMPORTANT]
> **Top 5 Changes in Release 2.3.0:**
> 1. **Style node:** the look as dropdowns instead of free text — ten black & white and seven colour looks, each measured to render differently, plus genre, lens and finish ([Black & White Matrix](docs/POSTERS.md#black--white-looks-matrix-10-looks), [Colour Matrix](docs/POSTERS.md#colour-looks-matrix-7-looks)).
> 2. **Real black & white:** `{style}` moves to the front of the prompt, where a colour treatment actually takes, and a Monochrome node after VAE Decode removes the last trace of colour ([why](docs/measurements.md), [FAQ](#faq)).
> 3. **Start number for the series:** a die next to the photo count — a new number is a new series with the same settings, no prompt edit needed.
> 4. **The counter is the photo number, not a seed:** the widget is called `photo` now and counts from 1; the noise seed leaves on `image_seed` as before ([FAQ](#faq)).
> 5. **Tidied example workflows:** five numbered groups, the Style node in place of the text box, the Monochrome node in the output path with a note that says where to switch it on, and thumbnails in ComfyUI's template browser.
>
...
> Qwen-VL at 8 steps, CFG 1.0, no LoRA; another model will read the same words
> differently. The one node that touches pixels is *Photoshoot Monochrome*.
...
8 steps and CFG 1. Every measurement in [docs/measurements.md](docs/measurements.md)
