...
A character LoRA fits in here too, if you have one — `LoraLoaderModelOnly`
between the `UNETLoader` and the sampler, nothing else changes. You do not need
one for this, though. The noise seed alone holds a person still.
...
to `krea2/anchor` so they do not mix with a series.
